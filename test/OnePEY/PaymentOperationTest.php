...
class PaymentOperationTest extends TestCase {
  ...
  public function test_endpoint() {
    ...
    $reflection = new \ReflectionClass('OnePEY\PaymentOperation');
  ...
  public function test_buildRequestMessage() {
    ...
    $arr = array(
    		...
    		'description' => 'payment test '.substr(self::getCurrentPhpVer(),0,3).'-'.date('YmdHi'),
    );
  ...
  protected function getTestObject($threed = false) {
    ...
    $transaction->setDescription('payment test '.substr(self::getCurrentPhpVer(),0,3).'-'.date('YmdHi'));
