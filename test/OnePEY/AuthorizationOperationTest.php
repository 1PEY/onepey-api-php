...
class AuthorizationOperationTest extends TestCase {
  ...
  public function test_setDescription() {
    ...
    $description = 'Test desc '.substr(self::getCurrentPhpVer(),0,3).'-'.date('YmdHi');
  ...
  public function test_setTrackingId() {
    $auth = $this->getTestObjectInstance();
    $tracking_id = 'Test tracking_id '.substr(self::getCurrentPhpVer(),0,3).'-'.date('YmdHi');
  ...
  public function test_buildRequestMessage() {
    ...
    $arr = array(
    		'amount' => 12.33,
    		'currency' => 'USD',
    		'orderID' => 'TRACK-'.substr(self::getCurrentPhpVer(),0,3).'-'.date('YmdHi'),
    		'returnURL' => '',
    		'notifyURL' => '',
    		...
    		'description' => 'test auth '.substr(self::getCurrentPhpVer(),0,3).'-'.date('YmdHi'),
    );
    
    $reflection = new \ReflectionClass( 'OnePEY\AuthorizationOperation');
  ...
  protected function getTestObject($threed = false) {
    ...
    $transaction->setDescription('test auth '.substr(self::getCurrentPhpVer(),0,3).'-'.date('YmdHi'));
    $transaction->setTrackingId('TRACK-'.substr(self::getCurrentPhpVer(),0,3).'-'.date('YmdHi'));
